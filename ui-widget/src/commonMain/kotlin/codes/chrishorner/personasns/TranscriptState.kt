...
class TranscriptState internal constructor(
    ...
    private val messagesState = MessagesState(chatSession)
    ...
    private fun createEntryState(index: Int, message: ChatMessage): EntryState = with(density) {
        ...
        return EntryState(
            ...
            punctuationScale = Animatable(0f).apply {
                if (message.emotionMarker != EmotionMarker.EMOTION_MARKER_NONE && message.emotionMarker != EmotionMarker.EMOTION_MARKER_UNKNOWN) {
                    coroutineScope.launch {
    ...
    private fun finalizeEntryState(entryState: EntryState) = with(density) {
        ...
        val horizontalOffset = when {
            entryState.message.fromSelf -> Offset(0f, 0f)
