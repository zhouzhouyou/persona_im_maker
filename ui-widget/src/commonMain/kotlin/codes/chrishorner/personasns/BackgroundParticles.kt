 ...
 */
@Composable
fun BackgroundParticles(particle: BackgroundParticle, modifier: Modifier = Modifier) {
    val state = remember { ParticlesState() }
    state.particle = particle

    if (particle == BackgroundParticle.NONE) return

    val frameTimeMillis by rememberFrameTimeMillis()
    val particlePaths = rememberParticlePaths(particle)
...
@Composable
private fun rememberParticlePaths(particle: BackgroundParticle): List<ParticlePath> {
    val density = LocalDensity.current
    return remember(density, particle) {
        when (particle) {
            BackgroundParticle.NONE, BackgroundParticle.UNKNOWN -> emptyList()
            BackgroundParticle.SAKURA -> listOf(
            ...
            BackgroundParticle.SNOWFLAKE -> listOf(
...
private class ParticlesState {
    ...
    var particle = BackgroundParticle.NONE
        set(value) {
            if (field != value) {
                field = value
                initialized = false
    ...
    private fun spawnParticle(worldSize: DpSize, spawnInBounds: Boolean = false) {
        ...
        particle.apply {
            ...
            color = when (this@ParticlesState.particle) {
                BackgroundParticle.NONE, BackgroundParticle.UNKNOWN -> Color.Unspecified
                BackgroundParticle.SAKURA -> if (Random.nextBoolean()) Color(0xFFF2657B) else Color(0xFFE7354A)
                BackgroundParticle.SNOWFLAKE -> if (Random.nextBoolean()) Color(0xFF730D00) else Color(0xFF9F0B00)
            }
            x = when {
